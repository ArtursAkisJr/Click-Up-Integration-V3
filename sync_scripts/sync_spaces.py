...
import logging
from supabase_upload.spaces_upload import upload_spaces_to_supabase
import psycopg2
import os
...
def main():
    try:
        # Run get_spaces.py and capture its output
        result = subprocess.run(
            [sys.executable, str(Path(__file__).parent.parent / 'clickup_api' / 'get_spaces.py')],
            check=True,
            capture_output=True,
            text=True
        )
        ...
        sync_status_id, sync_status_timestamp = update_sync_status('spaces', 'SUCCESS', len(spaces))
        # Upload to Supabase with sync_status_id and timestamp
        upload_spaces_to_supabase(spaces, sync_status_id, sync_status_timestamp)
        logger.info('Spaces sync completed successfully.')
    except Exception as e:
        update_sync_status('spaces', 'ERROR', 0, str(e))
        logger.error(f'Error in sync_spaces: {e}')
        sys.exit(1)
