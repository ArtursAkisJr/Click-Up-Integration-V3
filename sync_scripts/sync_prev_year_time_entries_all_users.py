...
def transform_time_entry(entry):
    ...
    def ms_to_datetime(ms):
        return (
            datetime.fromtimestamp(int(ms) / 1000, tz=timezone.utc) if ms else None
        )
    ...
    entry["start_datetime"] = (
        ms_to_datetime(entry.get("start")).isoformat() if entry.get("start") else None
    )
    entry["end_datetime"] = (
        ms_to_datetime(entry.get("end")).isoformat() if entry.get("end") else None
    )
    return entry
...
def main():
    try:
        logger.info("Cleaning up existing duplicate time entries before previous-year sync...")
        ...
        logger.info(f"Syncing previous year time entries for year {year} ({start_date} to {end_date})...")

        result = subprocess.run(
            [
                sys.executable,
                str(Path(__file__).parent.parent / "clickup_api" / "get_time_entries.py"),
                "--user_ids",
                *user_ids,
                "--start_date",
        ...
        print(f"Fetched {len(time_entries)} previous-year time entries.")

        sync_status_id, sync_status_timestamp = update_sync_status(
            "time_entries_prev_year", "SUCCESS", len(time_entries)
        )
        ...
        gc.collect()
        logger.info("Previous-year time entries sync completed successfully.")
    except Exception as e:
        update_sync_status("time_entries_prev_year", "ERROR", 0, str(e))
        logger.error(f"Error in sync_prev_year_time_entries_all_users: {e}")
        sys.exit(1)
