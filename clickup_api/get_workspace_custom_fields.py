import os
import requests
from dotenv import load_dotenv
import logging
import sys
import json
from datetime import datetime
from pathlib import Path

# Set up logging
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger(__name__)

class ClickUpCustomFields:
    def __init__(self):
        logger.info("Initializing ClickUpCustomFields")
        # Load environment variables from the project root
        load_dotenv(dotenv_path=Path(__file__).parent.parent / 'env_real.env', override=True)
        self.clickup_api_key = os.getenv('CLICKUP_API_KEY')
        self.clickup_team_id = os.getenv('CLICKUP_TEAM_ID')
        if not self.clickup_api_key:
            raise ValueError("CLICKUP_API_KEY not found in environment variables")
        if not self.clickup_team_id:
            raise ValueError("CLICKUP_TEAM_ID not found in environment variables")
        self.headers = {
            'Authorization': self.clickup_api_key,
            'Content-Type': 'application/json'
        }
    def get_custom_fields(self):
        """
        Get all workspace-level custom fields from ClickUp
        Returns:
            list: List of dictionaries containing custom field information
        """
        url = f'https://api.clickup.com/api/v2/team/{self.clickup_team_id}/field'
        try:
            logger.info(f"Fetching custom fields from {url}")
            response = requests.get(url, headers=self.headers)
            response.raise_for_status()
            data = response.json()
            fields = data.get('fields', [])
            logger.info(f"Successfully fetched {len(fields)} custom fields")
            return fields
        except requests.exceptions.RequestException as e:
            logger.error(f"Failed to fetch custom fields: {str(e)}")
            if hasattr(e, 'response') and e.response is not None:
                logger.error(f"Response status code: {e.response.status_code}")
                logger.error(f"Response text: {e.response.text}")
            raise

def main():
    try:
        fields_client = ClickUpCustomFields()
        fields = fields_client.get_custom_fields()
        print("\nClickUp Workspace Custom Fields:")
        print("-" * 80)
        for field in fields:
            print(f"ID: {field.get('id')}")
            print(f"Name: {field.get('name')}")
            print(f"Type: {field.get('type')}")
            print(f"Required: {field.get('required')}")
            print("-" * 80)
        # Save to JSON file in clickup_api_response
        response_dir = Path(__file__).parent.parent / 'clickup_api_response'
        response_dir.mkdir(exist_ok=True)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_file = response_dir / f'get_custom_fields_{timestamp}.json'
        with open(output_file, 'w') as f:
            json.dump(fields, f, indent=2)
        logger.info(f"Custom fields data saved to {output_file}")
    except Exception as e:
        logger.error(f"Error in main: {str(e)}")
        sys.exit(1)

if __name__ == "__main__":
    main() 