...
# Fetch all space ids from Supabase
def get_all_spaces():
...
def fetch_folderless_lists_for_space(space_id):
    ...
    try:
        ...
        logger.error(f"Failed to fetch folderless lists for space {space_id}: {str(e)}")
        return []

def flatten_list(list_obj, space_id, space_name):
    ...
    flat = {
        ...
        'statuses': list_obj.get('statuses'),  # Can be stored as JSONB
        ...
        'custom_fields': list_obj.get('custom_fields'),  # Can be stored as JSONB
    }
    return flat
...
def main():
    try:
        ...
        for space in all_spaces:
            lists = fetch_folderless_lists_for_space(space['id'])
            for lst in lists:
                flat = flatten_list(lst, space['id'], space['name'])
        ...
        output_file = response_dir / f'get_lists_folderless_{timestamp}.json'
        with open(output_file, 'w') as f:
            json.dump(all_lists, f, indent=2)
        logger.info(f"Folderless lists data saved to {output_file}")
